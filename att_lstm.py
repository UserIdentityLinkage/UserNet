...
class Att_LSTM:
    def __init__(self, embedding_size, embedding_img, hidden_size1=75,  batch_size1=32):
        self.input_x = tf.placeholder(tf.float32, [None, None, embedding_size], name='input_x')
        self.input_image = tf.placeholder(tf.float32, [None, None, embedding_img], name='input_x')
        self.batch_size = batch_size1

        self.word_embeddings = embedding_size
        self.hidden_size = hidden_size1
        self.output1 = self.Bilstm(self.input_x, 'Ins')

        self.output = self.output1
        self.insimg = layers.fully_connected(self.input_image,self.hidden_size*2)
        
        # self.FC(self.output_attention, 'FC')
